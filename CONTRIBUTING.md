...
1. **The Performance Moat**: Core logic should avoid heavy external dependencies (preferring standard library utilities like `std::sync::OnceLock` instead of external crates like `once_cell`) that incur a startup tax. *(Exception: The `regex` crate is used for the redaction engine, but its patterns must be compiled exactly once via `std::sync::OnceLock` to mitigate initialization costs.)* We aim for sub-20ms total execution time.
2. **Concurrency Safety**: All file-write operations must use **Universal Advisory Locking (`fd-lock`)** and atomic in-place modification logic to ensure data integrity across parallel sessions.
...
4. **Best-Effort Sanitization**: Common credential and secret patterns should be redacted using the regex sieve *before* they are logged to disk.
