...
if (-not (Test-Path (Join-Path $termimDir "projects"))) { New-Item -ItemType Directory -Path (Join-Path $termimDir "projects") -Force | Out-Null }
if (-not (Test-Path (Join-Path $termimDir "registry.txt"))) { New-Item -ItemType File -Path (Join-Path $termimDir "registry.txt") -Force | Out-Null }
...
Write-Host "  Termim v1.2.6 Installed Successfully" -ForegroundColor Green
